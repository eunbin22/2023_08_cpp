...
int main() {
	...
	while (true) {
		...
		if (input == 1) {
			printf("== 전체 사원 보기 ==\n");
			for (int i = 0; i < count; i++) { // 전체 배열 순회 // 어디까지 채워져있는지 모르니까 count로 기억
				printf("== %d 번째 사원 == \n", i+1);
				employee[i]->printInfo(); // 순회하면서 함수 실행
			}
			...
			for (int i = 0; i < count; i++) { // 배열을 순회하는 반복문
				if (number == employee[i]->no) { // 사용자가 방금 입력한 사원 번호와 배열 안의 어떤 사원 번호가 일치하면 
					delete employee[i]; // 일치하는 사원을 삭제시키겠다.
					deletedIndex = i; // 일치하는 사원이 있을 경우 deletedIndex에다가 해당 사원의 인덱스를 덮어쓰기
					break;
				}
			}
			if (deletedIndex >= 0) { // 삭제가 됐네? // 누군가 삭제가 된 경우 앞으로 한 칸씩 움직여야함.
